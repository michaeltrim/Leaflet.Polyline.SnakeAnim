...
L.Polyline.include({
  ...
  snakeIn: function() {
    ...
    this._hasFiredSnakeWillEnd = false
  ...
  _snakeForward: function(forward) {
    ...
    if (this._map) {
      ...
      if (this._snakingDistance + forward > distance) {
        ...
        } else if (this._snakingVertices >= this._snakeLatLngs[this._snakingRings].length - 3) {
          this.fire('snakewillend')
        }
      ...
      this._percent = this._snakingDistance / distance

      // console.log(this._percent,this._hasFiredSnakeWillEnd)
      // if(this._percent>.7 && !this._hasFiredSnakeWillEnd){

      // 	this._hasFiredSnakeWillEnd = true;
      // }

      var headPoint = nextPoint.multiplyBy(this._percent).add(currPoint.multiplyBy(1 - this._percent))
